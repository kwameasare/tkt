...
import 'package:tkt/widgets/verifyticket_item.dart';
...
class AllVerifyTickets extends StatefulWidget {
  final String phone;
  const AllVerifyTickets(this.phone);
  @override
  _AllVerifyTicketsState createState() => _AllVerifyTicketsState();
}

class _AllVerifyTicketsState extends State<AllVerifyTickets> {
  ...
  Future<List<Category>> _queryCategory() async {
    ...
    String url = "$baseUrl/verifiers?filter[userPhone]=+233${widget.phone}";
    ...
    if (response.statusCode == 200) {
      ...
      print("starting array input1");
      print(convertedResponse["data"]);
      convertedResponse["data"].forEach((data) async {
        var event_id = data["event_id"];

        String finalUrl = '$baseUrl/verifiers?filter[event_id]=$event_id';

        Response scd_response = await http.get(finalUrl, headers: headers);
        if (scd_response.statusCode == 200) {
          var convertedResponse = json.decode(scd_response.body);
          Category filterEvent = new Category(
            name: convertedResponse["data"][0]["name"],
            organiser: convertedResponse["data"][0]['organizer'],
            banner: convertedResponse["data"][0]['banner'],
            location: convertedResponse["data"][0]['venue'],
            description: convertedResponse["data"][0]['description'],
            pricing: convertedResponse["data"][0]['pricing'],
            // sessions: convertedResponse["data"][0]['sessions'].toList(),
            lat: convertedResponse["data"][0]["venue_gps_lat"],
            lng: convertedResponse["data"][0]["venue_gps_lng"],
            venue: convertedResponse["data"][0]["venue"],
            ticket_types: convertedResponse["data"][0]["ticket_types"].toList(),
            event_id: convertedResponse["data"][0]["id"],
